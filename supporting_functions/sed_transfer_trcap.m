function [Qbi_tr_t, Q_out_t , setplace, perc_out] = sed_transfer_trcap(V_mob , n , v_sed_day , Lngt, Network )
% SED_TRANSFER takes the matrix of the mobilized layers(V_mob) and the vector of
...
%% save results
    ...
    for c = 1:size(setplace,1)
        ...
        if ~isnan(roundpar)
           Qbi_tr_t(V_mob(:,1),:,c) = rounddown(V_mob(:,c+1) .* setplace(c,:), roundpar);
    ...
    if ~isnan(roundpar)
        Q_out_t(V_mob(:,1),:) = rounddown(V_mob(:,2:end) .* perc_out', roundpar);      
