...
flushing_successful = 0; %flusing_successful indicates if the flushing operation for the day was successful
...
for d=1:length(DamDatabase_active)
    ...
    elseif ResVolume_pre(dam_pos) + ( Q_input - DamDatabase_active(dam_pos).min_discharge)*(24*60*60)/1E6  < 0 % if the reservoir is empty, do not release anything
