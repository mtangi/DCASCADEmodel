function [] = plot_dam_features(dam_output,Q,dates_Q,varargin)
...
for d=1:n_dams
    ...
    %plot inflow 
    plot( datetime(dates_Q(:,1:ts)'),[Q(1:ts, DamDatabase(d).Node_ID)],'LineWidth',LineWidth)
    hold on
    ...
    if isempty(titles_subplot) 
        title([DamDatabase(d).Name ' - reach ' num2str(DamDatabase(d).Node_ID)])
    else      
